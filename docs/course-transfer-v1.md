...
The production transfer path uses the existing Nuxt client as both the desktop sender
and the browser receiver opened by the phone's system camera. The Cloudflare Worker only relays the WebRTC
offer, answer, and ICE candidates. Course JSON is sent directly over the ordered,
reliable WebRTC DataChannel.

## Deployment variables

The `Deploy course transfer signaling` workflow needs:
...
- The browser receiver intentionally omits remote login, ranking, sharing, daily
...
The QR code carries only the browser receiver URL, Signal Worker URL, and room token. The

<template>
  <section class="py-12 text-center">
    ...
    <h1 class="text-2xl font-bold">接收课程包</h1>
    <p
...
import { createCoursePackageFromCourse, upsertCoursePackage } from "~/services/courseRepository";
...
let savedCoursePack: LocalCoursePack | undefined;
...
function updateStatus(event: { status: TransferStatus; message?: string }): void {
  ...
  if (event.status === "completed" && savedCoursePack) {
    const coursePack = savedCoursePack;
    setTimeout(() => {
      void navigateTo(`/course-pack/${coursePack.id}`);
    }, 0);
  }
}
  ...
  try {
    session = createTransferSession({
      ...
      onCourseReceived: async (received) => {
        savedCoursePack = await upsertCoursePackage(
          createCoursePackageFromCourse(received.envelope.course),
        );
      },
      onCoursePackageReceived: async (received) => {
        savedCoursePack = await upsertCoursePackage(received.envelope.package);
      },
    });
